...
export default function SizeUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = sizeRecord
      ? { ...initialValues, ...sizeRecord }
      : initialValues;
    setUntitledfield(cleanValues.untitledfield);
    setErrors({});
  };
  const [sizeRecord, setSizeRecord] = React.useState(size);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Size, idProp) : size;
      setSizeRecord(record);
    };
    queryData();
  }, [idProp, size]);
  React.useEffect(resetStateValues, [sizeRecord]);
  const validations = {
    untitledfield: [],
  };
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Size.copyOf(sizeRecord, (updated) => {
